...
- Redelivery (LTL only): if redelivery_needed is true, or the tracking comments describe a failed delivery attempt, a rescheduled delivery, or a closed/unavailable consignee and there is no delivery_date, DO flag it. Name it a redelivery in "issue" (start the sentence with "Redelivery needed:"), set actionConfidence >= 0.85, and set actionTarget to "carrier" (the carrier must re-attempt) unless the consignee must schedule or confirm the appointment, in which case "customer". A redelivery is NOT resolved by an "Out For Delivery" status alone — only a delivery_date, or a status/comment that explicitly confirms delivery, resolves it.
- Storage risk (LTL only): if storage_risk is true, DO flag it even though nothing is "late". The freight is sitting at the carrier's destination terminal (since at_destination_since) and the carrier bills storage once the hold passes storage_hold_limit_hours; hold_hours_at_appointment is how long it will have sat by the booked appointment (hold_hours_so_far when no appointment is booked yet). Start "issue" with "Storage risk:" and state the hours and the appointment date. Set actionTarget to "customer" — the customer must either move the delivery appointment earlier or accept the storage charge — and actionConfidence >= 0.85. In "recommendation", say FPX should offer the customer both options and confirm the choice in writing. If storage_risk is false but storage_carrier_policy is true and hold_hours_at_appointment is within 12 hours of the limit, mention the exposure in "issue" without flagging.
